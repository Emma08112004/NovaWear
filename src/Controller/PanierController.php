...
public function ajoutPanier(int $id, Request $request, ProductRepository $productRepository, EntityManagerInterface $em): JsonResponse
{
    ...
    if ($existingItem) { // si article existe
        $existingItem->setQuantity($existingItem->getQuantity() + 1); // ajoute 1 à la quantité
    } else { // si article n'existe pas ca ajoute un autre produit au panier avce la quantité renseignée lors de l'ajout
        $panier = new Basket();
